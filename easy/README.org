...
func maximumProduct(nums []int) int {
	len := len(nums)
	// TODO: cache the index of the numbers while being sorted.
	// compare if its below zero and append - to it,
	sort.Ints(nums)
	fmt.Printf("nums: %v\n", nums)

	high := nums[len-1] * nums[len-2] * nums[len-3]
	low := nums[0] * nums[1] * nums[2]
	var output int
	if absInt(high) > absInt(low) {
		output = high
	} else {
		output = low
	}
...
// https://stackoverflow.com/a/68277627
...
func main() {
	...
	nums = []int{-100, -98, -1, 2, 3, 4} // Output: 24
	fmt.Printf("maximumProduct(): %v\n", maximumProduct(nums))
}
#+end_src
** Explanation
** Constraints
